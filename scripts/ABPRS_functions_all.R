...
Lasso_selection<-function(data_list,family = "binomial",lambda=NULL, bigdata=FALSE, sparse=FALSE){
  ...
  if(is.null(lambda)){
    fit0 <- cv.glmnet( x, dat_train,family = family)
    lambda<-fit0$lambda.min
  }
  if(bigdata){
    fit1 <- biglasso(x, dat_train, family = family,lambda=lambda)
    Theta<-fit1$beta  
  }else{
  fit1 <- glmnet(x, dat_train, family = family,lambda = lambda)
  Theta <- as.vector(coef(fit1))
  }
  support<-which(Theta[-c(1:2)]!=0)
  rm(Theta, fit1, x, theta_train)  # 释放内存
  gc() 
  return(support)
}
...
BH_selection<-function(data_list, family="binomial", alpha=0.1,lambda=3e-3){
  ...
  if(is.null(lambda)){
    ...
    # support<- which(fit0$beta[-1]!=0)
    # x<-as.matrix(cbind(prs_val,theta_val[,support]))
    # y<-dat_val
    # fit0 <- glmnet(x, y, family = "binomial",lambda =lambda)
    # Theta1 <- as.vector(coef(fit0))
    # fit0<-REF_DS_inf(x, y, family="binomial", lasso_est=Theta1, delta)
