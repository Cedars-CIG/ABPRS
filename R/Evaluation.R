...
tableHTML::tableHTML(PerformanceScores, rownames=FALSE, widths=c(150,150)),
...
<h2>Performance Score Table</h2>",
tableHTML::tableHTML(PerformanceScores, rownames=FALSE, widths=c(150,150, 150)),
"<button onclick=\"download_csv_file(csv1, filename1)\">Download PerformanceScores.csv</button>",
...
Plot_Score <- function(PerformanceScores, ylab){
  ...
  plot <- ggplot2::ggplot(data=PerformanceScores, ggplot2::aes(x=Model, y=Score, fill=Model)) +
    ggplot2::geom_bar(stat="identity", width=0.5) +
    ggplot2::geom_text(ggplot2::aes(label=round(Score, 2)), size=3.5, vjust=-0.3) +
    ggplot2::scale_y_continuous(expand = ggplot2::expansion(mult = c(0, .1))) + 
    ggplot2::labs(x="Model", y=ylab, fill="Model") +
...
AUC_Score<-function(prs, pheno){
  ...
  prediction <- ROCR::prediction(predict(mod, prs, type="response"), pheno)
...
Density_Plot <- function(PRSTable, model_names){
  ...
  plot<-ggplot2::ggplot(PRSTable,ggplot2::aes(x=Standardized_PRS, fill=Phenotype))+
    ggplot2::geom_density(alpha=0.5)+
    ggplot2::scale_fill_manual(values = c("#67a9cf", "#ef8a62"))+
    ggplot2::labs(x="Standardized PRS", y="Density") +
    ggplot2::theme(panel.grid.major = ggplot2::element_blank(), panel.grid.minor = ggplot2::element_blank(),
          panel.background = ggplot2::element_blank(), axis.line.x = ggplot2::element_line(colour = "black")) + 
    ggplot2::facet_grid(cols=ggplot2::vars(Method),scales="free_x")
  fig <- plotly::ggplotly(plot)
  return(fig)
}
...
Prevalence_Plot <- function(PrevalenceData){
  ...
  plot <- ggplot2::ggplot(PrevalenceData, ggplot2::aes(x = Percentile, y = Prevalence, color=Model)) +
    ggplot2::geom_point(size = 3) +
    ggplot2::labs(x = "Risk Score Percentile", y = "Percentage of Cases",
...
Mean_Plot <- function(MeanData){
  ...
  plot <- ggplot2::ggplot(MeanData, ggplot2::aes(x = Percentile, y = Mean, color=Model)) +
    ggplot2::geom_point(size = 3) +
    ggplot2::geom_errorbar(ggplot2::aes(ymin=Mean-SD, ymax=Mean+SD), width=0, alpha=0.8) +
    ggplot2::labs(x = "Risk Score Percentile", y = "Mean of Phenotype",
         title = "Mean of Phenotype vs. Risk Score Percentile")+
    ggplot2::theme(panel.grid.major = ggplot2::element_blank(), panel.grid.minor = ggplot2::element_blank(), 
          panel.background = ggplot2::element_blank(), axis.line = ggplot2::element_line(colour = "black")) 
  #ggplot2::scale_color_manual(values = setNames(c("#ef8a62", "#67a9cf"), c(name1, name2)))
  fig <- plotly::ggplotly(plot)
