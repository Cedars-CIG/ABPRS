...
adaptive_variable_selection<-function(pre_trained_prs, validation_prs, 
  ...
  # training AUC diff
  if(binary){
    prediction <- ROCR::prediction(predict(mod1, data=df_train_1, type="response"), training_phenotype)
    mod1_auc <- ROCR::performance(prediction,measure="auc")@y.values[[1]]
  }else{
  ...
  # validation AUC diff
  if(binary){
    prediction <- ROCR::prediction(predict(mod1, newdata=df_val_1, type="response"), validation_phenotype)
    mod1_val_auc <- ROCR::performance(prediction,measure="auc")@y.values[[1]]
  }else{
  ...
  if(biglasso){ 
    x<-biglasso::as.big.matrix(x)
    fit1 <- biglasso::biglasso(x, y, family = family,lambda=lam.seq, nlambda=nlambda)
    beta<-fit1$beta[-1,]
  }else{
    fit1 <- glmnet::glmnet(x, y, family = family,lambda=lam.seq, nlambda=nlambda)
    beta<-fit1$beta
  }
  ...
  for(i in 1:nlambda){
    ...
    if(length(support)>0){
      ...
      if(is.null(delta)){
        ...
        fit0 <- glmnet::glmnet(x, y, family = family,lambda =lam.seq[i])
      ...
      if(length(support1)>0){
        ...
        if(binary){
          # training AUC diff
          prediction <- ROCR::prediction(predict(mod2, data=df_train_2, type="response"), training_phenotype)
          mod2_auc <- ROCR::performance(prediction,measure="auc")@y.values[[1]]
          training_validation_t[i,1] <- mod2_auc - mod1_auc
          # validation AUC diff
          prediction <- ROCR::prediction(predict(mod2, newdata=df_val_2, type="response"), validation_phenotype)
          mod2_val_auc <- ROCR::performance(prediction,measure="auc")@y.values[[1]]
